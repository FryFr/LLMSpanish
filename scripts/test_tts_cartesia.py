"""Smoke test: stream text to CartesiaTTS and save the audio to WAV.

Usage:
    uv run python scripts/test_tts_cartesia.py
    uv run python scripts/test_tts_cartesia.py "Tu frase acá"
"""
...
async def main(fragments: list[str], out: Path) -> None:
    ...
    if not settings.cartesia_api_key:
        print("ERROR: CARTESIA_API_KEY not set in .env", file=sys.stderr)
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    asyncio.run(main(frags, Path("assets/test/tts_out.wav")))
